...
//
// Mapa de columnas (0-indexed) — 27 columnas totales (A–AA):
//  0  Marca temporal
...
//  ── Columnas manuales GoTrack (cols O–AA) ────────────────────
// 14  ESTADO_SHEET         (O)
// 15  FECHA_CITA           (P)
// 16  HORA_CITA            (Q)
// 17  SIN_CITA             (R)
...
// 25  SUBSANACION_URL      (Z — URL Drive del documento subsanado subido por Comercial)
// 26  OBSERVACION_CONTENIDO (AA — observación de datos de formulario registrada por Legal)
...
export function parsearVentas(filas) {
  ...
  return filas
    .map((fila, idx) => ({
      ...
      SUBSANACION_URL:         String(fila[25] || '').trim(),
      OBSERVACION_CONTENIDO:   String(fila[26] || '').trim(),
    }))
