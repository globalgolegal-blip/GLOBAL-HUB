...
import { hoyISO, desplazarISO, instanteLima, ahoraLima } from '../fechas'
...
export const ESTADO_CONFIG_VS = {
  ...
  INSCRITO:           { label:'Inscrito en RRPP', labelCorto:'Inscrito', colorText:'#166534', bgBadge:'#DCFCE7', borderBadge:'#86EFAC', orden:11 },
}
...
export function puedeConfirmarCita(fecha, hora) { return !!(fecha && hora) }
...
export function tienePendienteParaRol(venta, rol) {
  ...
  if (rol === 'legal') return ((!!venta.GM_SOLICITADA&&!venta.GM_LEVANTADA) || (!!venta.FECHA_FIRMA&&!venta.FECHA_INSCRIPCION) || (estado==='DOCS_OBSERVADOS'&&areaObs==='LEGAL') || (estado==='DOCS_SUBSANADOS'&&areaObs==='LEGAL') || estado==='CONTENIDO_OBSERVADO')
  return false
}
