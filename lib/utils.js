export const ESTADOS = {
  ...
  VENCIDO:            'VENCIDO',
  VALIDADO:           'VALIDADO',
  PENDIENTE_JOTFORM:  'PENDIENTE_JOTFORM',   // (obsoleto — ya no se produce)
...
export function derivarEstado(contrato) {
  ...
  // ── Estados definitivos que cierran el contrato (ganan sobre el vencimiento) ──
  if (enviado === 'OBSERVADO') return ESTADOS.CONTRATO_OBSERVADO

  // Col P = SI: validado por trigger o por Legal → VALIDADO.
  if (firma === 'SI') return ESTADOS.VALIDADO

  // Col V = CONFORME: validación automática (cubre el lapso hasta que el trigger pone P=SI).
  if (resultado === 'CONFORME') return ESTADOS.VALIDADO

  if (firma === 'VENCIDO') return ESTADOS.VENCIDO

  // ── Vencimiento por fecha — ANTES de observado ───────────────────────────────
  // El plazo para subsanar una observación ES el plazo de firma: si la fecha de
  // vencimiento ya pasó, el contrato es VENCIDO aunque esté observado (por firma
  // o por sistema). Solo lo definitivo (validado/conforme) le gana al vencimiento.
  ...
  // ── Observaciones vigentes (aún dentro del plazo de firma) ───────────────────
...
// OBSERVADO_SISTEMA aparece como OBSERVADO para el operador.
