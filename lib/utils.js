...
export function derivarEstado(contrato) {
  ...
  // Col P = SI: validado por trigger o por Legal.
  // Col W = PENDIENTE → Legal aún no ha confirmado JotForm → mostrar PENDIENTE_JOTFORM.
  // Col W = COMPLETADO o vacío → totalmente validado → VALIDADO.
  if (firma === 'SI') {
    const jotform = (contrato['JOTFORM'] || '').trim().toUpperCase()
    if (jotform === 'PENDIENTE') return ESTADOS.PENDIENTE_JOTFORM
    return ESTADOS.VALIDADO
  }

  if (firma === 'OBSERVADO')   return ESTADOS.OBSERVADO
  if (firma === 'VENCIDO')     return ESTADOS.VENCIDO

  // ── Col V: resultado de validación automática (solo cuando col P aún no es SI) ──
  // CONFORME ya fue procesado por el trigger → col P = SI → rama de arriba lo captura.
  // Este bloque cubre el lapso entre que TI escribe y el trigger corre (máx 5 min).
...
export function estadoParaVista(estado, contrato) {
  // El operador no ve estados internos de Legal — solo versiones simplificadas:
  // PENDIENTE_JOTFORM → VALIDADO  (para el operador el contrato ya está validado;
  //                                el paso JotForm es un proceso interno de Legal)
  // OBSERVADO_SISTEMA → OBSERVADO (hay un problema detectado por el sistema)
  if (estado === ESTADOS.PENDIENTE_JOTFORM) return ESTADOS.VALIDADO
  if (estado === ESTADOS.OBSERVADO_SISTEMA) return ESTADOS.OBSERVADO
  return estado
}
